...
test.describe('Admin: Students Management', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await expect(page).toHaveURL(/.*\/admin\/users\/students/, { timeout: 15000 });
    });

    test('should load the students list and have add button', async ({ page }) => {
        await expect(page.locator('h1', { hasText: 'Students' })).toBeVisible({ timeout: 15000 });
        await expect(page.getByRole('link', { name: /Add Student/i })).toBeVisible();
    });

    test('should create a new student successfully', async ({ page }) => {
        await page.goto('/admin/users/students/create');
        ...
        const firstName = `StudentF${uniqueSuffix}`;
        const lastName = `StudentL${uniqueSuffix}`;
        const testPhone = `98765${uniqueSuffix.slice(-5)}`; // 10 digit mock
        const testAdmissionId = `ADM-${uniqueSuffix}`;
        ...
        // Admission Details
        await page.getByPlaceholder('Admission ID').fill(testAdmissionId);
        ...
        await expect(page.getByText('Student created successfully')).toBeVisible({ timeout: 30000 });

        // Should return to the list
        await expect(page).toHaveURL(/\/admin\/users\/students$/, { timeout: 30000 });

        // Verify the created student appears in the list by searching by name
        const searchInput = page.locator('input[type="search"], input[placeholder*="Search students"]').first();
