...
// Get available departments to generate (those not already created)
export async function getAvailableDepartmentTemplates() {
  try {
    ...
    const available = STANDARD_DEPARTMENTS.filter(
      d => !existingNames.has(d.name.toLowerCase())
    );

    return { success: true, data: available };
  } catch (error) {
    console.error("Error fetching available department templates:", error);
    return {
      success: false,
...
// Auto-generate selected departments
export async function autoGenerateDepartments(selectedNames?: string[]) {
  try {
    ...
    // Filter departments to create
    let departmentsToCreate = STANDARD_DEPARTMENTS.filter(
      d => !existingNames.has(d.name.toLowerCase())
    );
    ...
    if (selectedNames && selectedNames.length > 0) {
      ...
      departmentsToCreate = departmentsToCreate.filter(
        d => selectedSet.has(d.name.toLowerCase())
      );
    }
    ...
    if (departmentsToCreate.length === 0) {
      return {
        success: false,
        error: "No new departments to create"
      };
    }
    ...
    return {
      success: true,
      count: result.count,
      message: `Created ${result.count} departments`
    };
  } catch (error) {
    console.error("Error auto-generating departments:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to auto-generate departments"
    };
  }
}
