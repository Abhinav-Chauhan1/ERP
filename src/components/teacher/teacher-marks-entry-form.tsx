...
import { ExportMarksDialog } from "@/components/admin/export-marks-dialog";
...
export function TeacherMarksEntryForm() {
  ...
  // Load all teacher-scoped data on mount
  useEffect(() => {
    let cancelled = false;
    ...
    const loadData = async () => {
      setIsLoading(true);
      try {
        const result = await getTeacherMarksEntryPageData();
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    if (isGloballyAssigned || hasWholeClassAssignment) {
      ...
      const assignedSections = assignments
        .filter((a) => a.classId === filterClassId && a.subjectId === filterSubjectId && a.sectionId)
        .map((a) => ({
          id: a.sectionId!,
          name: a.sectionName || "Unknown Section",
        }));

      // Deduplicate sections
      const uniqueSections = Array.from(
        new Map(assignedSections.map((s) => [s.id, s])).values()
      );
      setSections(uniqueSections);
    }
  }, [filterClassId, filterSubjectId, classes, assignments, globalSubjectIds]);
  ...
  const handleLoadStudents = async () => {
    ...
    // Call core Marks action using TEACHER context
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="space-y-1.5">
            ...
            >
              <SelectTrigger className={filterSubjectId ? "border-primary" : ""}>
                <SelectValue placeholder={filterClassId ? "Select subject" : "Select class first"} />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">
              Section
              {!filterSubjectId && (
                <span className="ml-1 text-xs text-muted-foreground/60">(select subject first)</span>
              )}
            </Label>
            <Select
              value={selectedSectionId}
              onValueChange={setSelectedSectionId}
              disabled={isLoading || !filterSubjectId || sections.length === 0}
            >
              <SelectTrigger className={selectedSectionId ? "border-primary" : ""}>
                <SelectValue placeholder={filterSubjectId ? "Select section" : "Select subject first"} />
              </SelectTrigger>
              <SelectContent>
          ...
          <CardContent className="pt-6">
            <div className="grid gap-4 md:grid-cols-4">
              ...
              <div>
                ...
                {selectedExam.examType.cbseComponent && (
                  <Badge variant="outline" className="text-xs bg-orange-50 text-orange-700 mt-1">
                    CBSE: {selectedExam.examType.cbseComponent}
                  </Badge>
                )}
              </div>
              <div>
          ...
          <CardContent>
            {studentsData.markConfig ? (
              ...
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  No mark configuration is set up for this exam. An administrator must configure mark components first.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
