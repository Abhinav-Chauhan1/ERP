...
import { getBookFeeAnalyticsByClass, type BookFeeClassAnalytics } from "@/lib/actions/miscFeeActions";
import { formatCurrency } from "@/lib/utils/export-utils";
import toast from "react-hot-toast";

interface BookFeeAnalyticsProps {
  academicYears?: Array<{ id: string; name: string }>;
}

export function BookFeeAnalyticsComponent({ academicYears = [] }: BookFeeAnalyticsProps) {
  const [rows, setRows] = useState<BookFeeClassAnalytics[]>([]);
  ...
  const fetchAnalytics = useCallback(async function () {
    setLoading(true);
    try {
      const result = await getBookFeeAnalyticsByClass(academicYearId);
      if (result.success && result.data) {
        setRows(result.data);
      } else {
        toast.error(result.error || "Failed to fetch book fee analytics");
      }
    } catch (error) {
      console.error("Error fetching book fee analytics:", error);
      toast.error("Failed to fetch book fee analytics");
    } finally {
      setLoading(false);
    }
  }, [academicYearId]);
  ...
  const totals = rows.reduce(
    (acc, row) => ({
      studentsWithBookFee: acc.studentsWithBookFee + row.studentsWithBookFee,
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <CardTitle>Book Payments</CardTitle>
          <CardDescription>Class-wise breakdown of Books Fee collections</CardDescription>
        </div>
                ...
                <>
                  {rows.map((row) => (
                    <TableRow key={row.classId}>
                      ...
                      <TableCell className="text-right">{row.studentsWithBookFee}</TableCell>
                  ...
                  <TableRow className="font-semibold bg-muted/50">
                    ...
                    <TableCell className="text-right">{totals.studentsWithBookFee}</TableCell>
