import {
    Users,
    GraduationCap,
    LayoutGrid,
    BookOpen,
    ClipboardCheck,
    CreditCard,
    MessageSquare,
    FileText,
    BarChart3,
    Settings,
    School,
    PartyPopper,
    Library,
    Building2,
    CalendarDays,
    Bus,
    UserCheck,
    History,
} from "lucide-react";
import { RouteItem, SidebarConfig, HeaderConfig } from "./types";
import { PERMISSIONS } from "@/lib/constants/permissions";

export const adminRoutes: RouteItem[] = [
    // ── DAILY USE ────────────────────────────────────────────────────────────
    {
        label: "Dashboard",
        icon: LayoutGrid,
        href: "/admin",
    },
    {
        label: "Attendance",
        icon: ClipboardCheck,
        submenu: [
            { label: "Overview", href: "/admin/attendance" },
            { label: "Student Attendance", href: "/admin/attendance/students" },
            { label: "Teacher Attendance", href: "/admin/attendance/teachers" },
            { label: "Reports", href: "/admin/attendance/reports" },
        ],
        permissions: [PERMISSIONS.READ_ATTENDANCE],
    },
    {
        label: "Finance",
        icon: CreditCard,
        submenu: [
            { label: "Overview", href: "/admin/finance", permissions: [PERMISSIONS.READ_FEE, PERMISSIONS.READ_PAYMENT] },
            { label: "Fee Structure", href: "/admin/finance/fee-structure", permissions: [PERMISSIONS.READ_FEE] },
            { label: "Payments", href: "/admin/finance/payments", permissions: [PERMISSIONS.READ_PAYMENT] },
            { label: "Receipt Verification", href: "/admin/finance/receipt-verification", permissions: [PERMISSIONS.APPROVE_PAYMENT] },
            { label: "Expenses", href: "/admin/finance/expenses", permissions: [PERMISSIONS.READ_EXPENSE] },
            { label: "Scholarships", href: "/admin/finance/scholarships", permissions: [PERMISSIONS.READ_SCHOLARSHIP] },
            // GROWTH+
            { label: "Payroll", href: "/admin/finance/payroll", permissions: [PERMISSIONS.READ_PAYROLL], requiredFeature: "payroll" },
            { label: "Budget", href: "/admin/finance/budget", permissions: [PERMISSIONS.READ_BUDGET], requiredFeature: "budget" },
            { label: "Analytics", href: "/admin/finance/analytics", permissions: [PERMISSIONS.READ_PAYMENT], requiredFeature: "finance_analytics" },
        ],
        permissions: [PERMISSIONS.READ_FEE, PERMISSIONS.READ_PAYMENT],
    },
    {
        label: "Communication",
        icon: MessageSquare,
        submenu: [
            { label: "Overview", href: "/admin/communication" },
            { label: "Messages", href: "/admin/communication/messages" },
            { label: "Announcements", href: "/admin/communication/announcements" },
            { label: "Notifications", href: "/admin/communication/notifications" },
            { label: "Parent Meetings", href: "/admin/communication/parent-meetings" },
            // GROWTH+
            { label: "Bulk Messaging", href: "/admin/communication/bulk-messaging", requiredFeature: "bulk_messaging" },
            { label: "Templates", href: "/admin/communication/templates", requiredFeature: "message_templates" },
        ],
        permissions: [PERMISSIONS.READ_MESSAGE, PERMISSIONS.READ_ANNOUNCEMENT],
    },
    {
        label: "Users",
        icon: Users,
        submenu: [
            { label: "Overview", href: "/admin/users", permissions: [PERMISSIONS.READ_USER] },
            { label: "Administrators", href: "/admin/users/administrators", permissions: [PERMISSIONS.READ_USER] },
            { label: "Teachers", href: "/admin/users/teachers", permissions: [PERMISSIONS.READ_TEACHER] },
            { label: "Students", href: "/admin/users/students", permissions: [PERMISSIONS.READ_STUDENT] },
            { label: "Parents", href: "/admin/users/parents", permissions: [PERMISSIONS.READ_PARENT] },
        ],
        permissions: [PERMISSIONS.READ_USER, PERMISSIONS.READ_STUDENT, PERMISSIONS.READ_TEACHER, PERMISSIONS.READ_PARENT],
    },

    // ── WEEKLY USE ───────────────────────────────────────────────────────────
    {
        label: "Assessment",
        icon: BookOpen,
        submenu: [
            { label: "Overview",          href: "/admin/assessment",                permissions: [PERMISSIONS.READ_EXAM] },
            { label: "Exam Setup",        href: "/admin/assessment/exams",          permissions: [PERMISSIONS.READ_EXAM] },
            { label: "Marks Entry",       href: "/admin/assessment/marks-entry",    permissions: [PERMISSIONS.CREATE_MARKS] },
            { label: "Results & Reports", href: "/admin/assessment/results",        permissions: [PERMISSIONS.READ_RESULT] },
            { label: "Report Cards",      href: "/admin/assessment/report-cards",   permissions: [PERMISSIONS.READ_RESULT] },
            { label: "Activities",        href: "/admin/assessment/activities",     permissions: [PERMISSIONS.READ_ASSIGNMENT, PERMISSIONS.READ_MARKS] },
        ],
        permissions: [PERMISSIONS.READ_EXAM, PERMISSIONS.READ_ASSIGNMENT],
    },
    {
        label: "Academic",
        icon: GraduationCap,
        submenu: [
            { label: "Overview", href: "/admin/academic" },
            { label: "Academic Years", href: "/admin/academic/academic-years" },
            { label: "Terms", href: "/admin/academic/terms" },
            { label: "Grades", href: "/admin/academic/grades" },
            { label: "Subjects", href: "/admin/teaching/subjects" },
            { label: "Syllabus", href: "/admin/academic/syllabus" },
            { label: "Timetable", href: "/admin/teaching/timetable" },
            { label: "Promotion", href: "/admin/academic/promotion" },
        ],
        permissions: [PERMISSIONS.READ_CLASS, PERMISSIONS.READ_SUBJECT],
    },
    {
        label: "Classes",
        icon: School,
        submenu: [
            { label: "Class List", href: "/admin/classes" },
            { label: "Sections", href: "/admin/classes/sections" },
            { label: "Rooms", href: "/admin/classes/rooms" },
        ],
        permissions: [PERMISSIONS.READ_CLASS],
    },
    {
        label: "Admissions",
        icon: GraduationCap,
        href: "/admin/admissions",
        permissions: [PERMISSIONS.READ_APPLICATION],
        requiredFeature: "admissions",
    },

    // ── MONTHLY / RARE ───────────────────────────────────────────────────────
    {
        label: "Reports",
        icon: BarChart3,
        submenu: [
            { label: "Overview", href: "/admin/reports" },
            { label: "Academic Reports", href: "/admin/reports/academic" },
            { label: "Attendance Reports", href: "/admin/reports/attendance" },
            { label: "Financial Reports", href: "/admin/reports/financial" },
            // GROWTH+
            { label: "Performance Analytics", href: "/admin/reports/performance", requiredFeature: "advanced_reports" },
            { label: "Report Builder", href: "/admin/reports/builder", requiredFeature: "advanced_reports" },
            { label: "Scheduled Reports", href: "/admin/reports/scheduled", requiredFeature: "advanced_reports" },
            { label: "Comparative", href: "/admin/reports/comparative", requiredFeature: "advanced_reports" },
        ],
        permissions: [PERMISSIONS.READ_REPORT],
    },
    {
        label: "Documents",
        icon: FileText,
        submenu: [
            { label: "Overview", href: "/admin/documents" },
            // GROWTH+
            { label: "ID Cards", href: "/admin/id-cards", requiredFeature: "id_cards" },
            { label: "Certificates", href: "/admin/certificates", requiredFeature: "certificates" },
        ],
        permissions: [PERMISSIONS.READ_DOCUMENT, PERMISSIONS.READ_CERTIFICATE],
    },
    {
        label: "Calendar",
        icon: CalendarDays,
        href: "/admin/calendar",
    },
    {
        label: "Events",
        icon: PartyPopper,
        href: "/admin/events",
    },
    {
        label: "Library",
        icon: Library,
        submenu: [
            { label: "Overview", href: "/admin/library" },
            { label: "Books", href: "/admin/library/books" },
            { label: "Reports", href: "/admin/library/reports" },
        ],
        permissions: [PERMISSIONS.READ_BOOK],
        requiredFeature: "library",
    },
    {
        label: "Transport",
        icon: Bus,
        submenu: [
            { label: "Overview", href: "/admin/transport" },
            { label: "Vehicles", href: "/admin/transport/vehicles" },
            { label: "Routes", href: "/admin/transport/routes" },
            { label: "Attendance", href: "/admin/transport/attendance" },
        ],
        permissions: [PERMISSIONS.READ_VEHICLE, PERMISSIONS.READ_ROUTE],
        requiredFeature: "transport",
    },
    {
        label: "Hostel",
        icon: Building2,
        submenu: [
            { label: "Overview", href: "/admin/hostel" },
            { label: "Rooms", href: "/admin/hostel/rooms" },
            { label: "Visitors", href: "/admin/hostel/visitors" },
            { label: "Fees", href: "/admin/hostel/fees" },
            { label: "Complaints", href: "/admin/hostel/complaints" },
        ],
        requiredFeature: "hostel",
    },
    {
        label: "Alumni",
        icon: UserCheck,
        submenu: [
            { label: "Directory", href: "/admin/alumni" },
            { label: "Communication", href: "/admin/alumni/communication" },
            { label: "Statistics", href: "/admin/alumni/statistics" },
        ],
        permissions: [PERMISSIONS.READ_ALUMNI],
        requiredFeature: "alumni",
    },
    {
        label: "Settings",
        icon: Settings,
        submenu: [
            { label: "Billing & Plan", href: "/admin/settings/billing" },
            { label: "Choose / Upgrade Plan", href: "/admin/settings/plan" },
            { label: "General", href: "/admin/settings" },
            { label: "Branding", href: "/admin/settings/branding" },
            { label: "Permissions", href: "/admin/settings/permissions" },
            { label: "Notifications", href: "/admin/settings/notifications" },
            { label: "Two-Factor Auth", href: "/admin/settings/two-factor" },
            { label: "Payment Config", href: "/admin/settings/payment-configuration" },
            { label: "WhatsApp Profile", href: "/admin/communication/whatsapp-profile", requiredFeature: "whatsapp" },
            { label: "Sessions", href: "/admin/settings/sessions" },
            { label: "Backups", href: "/admin/settings/backups" },
        ],
        permissions: [PERMISSIONS.READ_SETTINGS, PERMISSIONS.UPDATE_SETTINGS],
    },
    {
        label: "Audit Logs",
        icon: History,
        href: "/admin/audit-logs",
        permissions: [PERMISSIONS.READ_SETTINGS],
        requiredFeature: "audit_logs",
    },
];

export const adminSidebarConfig: SidebarConfig = {
    routes: adminRoutes,
    portalLabel: "Admin Portal",
    dashboardHref: "/admin",
    accountLabel: "Admin Account",
};

export const adminHeaderConfig: HeaderConfig = {
    dashboardHref: "/admin",
    pageTitleMappings: {
        "/admin/users": "Users",
        "/admin/academic": "Academic",
        "/admin/classes": "Classes",
        "/admin/teaching": "Teaching",
        "/admin/assessment": "Assessment",
        "/admin/attendance": "Attendance",
        "/admin/finance": "Finance",
        "/admin/communication": "Communication",
        "/admin/library": "Library",
        "/admin/transport": "Transport",
        "/admin/hostel": "Hostel",
        "/admin/admissions": "Admissions",
        "/admin/alumni": "Alumni",
        "/admin/calendar": "Calendar",
        "/admin/events": "Events",
        "/admin/documents": "Documents",
        "/admin/id-cards": "ID Cards",
        "/admin/certificates": "Certificates",
        "/admin/reports": "Reports",
        "/admin/audit-logs": "Audit Logs",
        "/admin/settings": "Settings",
        "/admin": "Dashboard",
    },
};
