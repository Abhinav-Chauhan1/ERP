  ...
  const loadDashboardData = useCallback(async function () {
    try {
      ...
      const [
        ...
        getRealTimeMetrics(),
        getChannelHealth(),
        getErrorLogsAction({ limit: 10, resolved: false }),
        getErrorStatisticsAction(last7Days, now),
        getErrorTimeSeriesData(last7Days, now),
        getMessageTimeSeriesData(last7Days, now),
      ]);
      ...
      setErrorTimeSeries(errorTimeSeriesData);
      setMessageTimeSeries(messageTimeSeriesData);
    } catch (error: any) {
      toast({
        title: 'Error',
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics.messagesLast24Hours.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              {metrics.messagesLastHour} in last hour
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics.errorsLast24Hours.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics.deliveryRateLast24Hours.toFixed(1)}%</div>
            <p className="text-xs text-muted-foreground">
              Avg: {metrics.averageDeliveryTime.toFixed(1)}s
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-500">{metrics.criticalErrors}</div>
            <p className="text-xs text-muted-foreground">
            ...
            {channelHealth.map((health) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium">{health.channel}</div>
                    <div className="text-sm text-muted-foreground">
                      {health.deliveryRate.toFixed(1)}% delivery
                    </div>
                  </div>
                </div>
                <Badge
        ...
        <TabsContent value="messages" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Daily message count by channel</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={messageTimeSeries}>
        ...
        <TabsContent value="stats" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {errorStats?.errorsByCategory.map((stat: any) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {errorStats?.errorsBySeverity.map((stat: any) => (
                    <div key={stat.severity} className="flex items-center justify-between">
                      <Badge
                        variant="outline"
                        style={{
                          borderColor: SEVERITY_COLORS[stat.severity as keyof typeof SEVERITY_COLORS],
                          color: SEVERITY_COLORS[stat.severity as keyof typeof SEVERITY_COLORS]
                        }}
                      >
                        {stat.severity}
                      </Badge>
                        ...
                        <div className="w-32 bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{
                              ...
                              backgroundColor: SEVERITY_COLORS[stat.severity as keyof typeof SEVERITY_COLORS]
                            }}
                          />
                        </div>
                        <span className="text-sm font-medium w-12 text-right">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {error.channel && (
                        <Badge
                          style={{
                            backgroundColor: CHANNEL_COLORS[error.channel as keyof typeof CHANNEL_COLORS]
                          }}
                        >
                          {error.channel}
                        </Badge>
