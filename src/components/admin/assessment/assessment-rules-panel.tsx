...
interface AssessmentRulesPanelProps {
  examTypes?: any[];
}

export function AssessmentRulesPanel({ examTypes: examTypesProp }: AssessmentRulesPanelProps) {
  const [rules, setRules] = useState<any[]>([]);
  const [classes, setClasses] = useState<any[]>([]);
  const [examTypes, setExamTypes] = useState<any[]>(examTypesProp || []);
  ...
  useEffect(() => {
    if (examTypesProp && examTypesProp.length > 0) {
      setExamTypes(examTypesProp);
    }
  }, [examTypesProp]);
  ...
  async function fetchData() {
    ...
    try {
      const [rulesRes, classesRes, examTypesRes, termsRes] = await Promise.all([
        getAssessmentRules(),
        getClasses(),
        examTypesProp && examTypesProp.length > 0 ? Promise.resolve({ success: true, data: examTypesProp }) : getExamTypes(),
        getTerms(),
      ]);
  ...
  function handleCreate() {
    form.reset({ name: "", classId: null, subjectId: null, termId: null, ruleType: "BEST_OF", examTypes: [], count: 2, weight: 1.0 });
    setEditingRule(null);
    setDialogOpen(true);
  }

  function handleEdit(rule: any) {
    form.reset({ name: rule.name, classId: rule.classId, subjectId: rule.subjectId, termId: rule.termId, ruleType: rule.ruleType, examTypes: rule.examTypes, count: rule.count, weight: rule.weight });
    setEditingRule(rule);
    setDialogOpen(true);
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">Grading Rules</h2>
          <p className="text-sm text-muted-foreground">Define how exam marks are aggregated for report cards</p>
        </div>
      ...
      <Alert className="bg-blue-50 border-blue-200">
        ...
        <AlertTitle className="text-blue-800">About Grading Rules</AlertTitle>
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {rules.length === 0 ? (
            <div className="col-span-3 text-center py-10 text-muted-foreground">
              ...
              <p className="text-sm mb-4">Create a grading rule to customize how report cards are calculated</p>
              <Button onClick={handleCreate}><PlusCircle className="mr-2 h-4 w-4" /> Add Rule</Button>
            </div>
          ) : (
            rules.map((rule) => (
              <Card key={rule.id}>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem className="text-red-600" onClick={() => { setRuleToDelete(rule.id); setDeleteDialogOpen(true); }}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{editingRule ? "Edit Rule" : "Create Grading Rule"}</DialogTitle>
            <DialogDescription>Define the aggregation logic for exam marks.</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="e.g. Best of 2 Periodic Tests" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="termId" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value || "all"}>
                      <FormControl><SelectTrigger><SelectValue placeholder="All Terms" /></SelectTrigger></FormControl>
                      <SelectContent>
              ...
              <FormField control={form.control} name="examTypes" render={() => (
                <FormItem>
                  ...
                  <div className="grid grid-cols-2 gap-2 border rounded-md p-3 max-h-[150px] overflow-y-auto">
                    {examTypes.map((type) => (
                      <FormField key={type.id} control={form.control} name="examTypes" render={({ field }) => (
                        <FormItem key={type.id} className="flex flex-row items-start space-x-3 space-y-0">
                          ...
                          <FormLabel className="text-sm font-normal">{type.name}</FormLabel>
                        </FormItem>
                      )} />
                    ))}
                  </div>
              ...
              <FormField control={form.control} name="weight" render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Relative weight in overall calculation (e.g., 0.3 for 30%).</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Grading Rule</DialogTitle>
            <DialogDescription>Are you sure? This will affect how report cards are calculated for the linked classes.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
