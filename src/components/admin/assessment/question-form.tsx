...
import { createSchoolQuestion, updateSchoolQuestion } from "@/lib/actions/adminQuestionBankActions";
...
export function QuestionForm({ question }: QuestionFormProps) {
    ...
    const loadSubjects = useCallback(async function () {
        ...
        if (result.success && result.data) {
            ...
            if (!question && result.data.length > 0) {
                setSubjectId(result.data[0].id);
            }
        }
    }, [question]);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            let result;
            if (question) {
                result = await updateSchoolQuestion(question.id, data);
            } else {
                result = await createSchoolQuestion(data);
            }
            ...
            if (result.success) {
                ...
                router.push("/admin/assessment/question-bank");
                router.refresh();
            } else {
