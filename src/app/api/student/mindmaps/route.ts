...
export async function GET(request: NextRequest) {
  // C28 FIX: Require authentication
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, message: error instanceof Error ? error.message : "Failed to get mind maps" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await createMindMap({ title, subject, nodes, connections, isPublic });
    
    return NextResponse.json(result);
  } catch (error) {
    console.error("Error in create mind map API:", error);
    return NextResponse.json(
      { success: false, message: error instanceof Error ? error.message : "Failed to create mind map" },
      { status: 500 }
    );
  }
}
